...
export class VipMixedPayment {
        ...
        try {
            ...
            if (cancel === 'Y') {
                ...
                for (let j = 0; j < payMethods.length; j++) {
                    //第几步取消
                    if (i === j) {
                        orderNoValue = await PaymentProcessing.cancelTheDeal(client);
                        LogUtils.log.info("取消交易：" + payMethods + "  符合预期");
                        break;
                    } else {
                        LogUtils.log.info("+++++++进入sales_cancelled")
            ...
            // 订单不取消
                ...
                LogUtils.log.info("销售：" + payMethods + "  符合预期");
            }
            ...
            // 出错截图
            await ScreenShotUtil.takeScreenShot(client, orderNoValue);
            //创建销售数据
            let saleData = new SaleData();
            saleData.saleTime = new Date().toLocaleDateString();
            saleData.orderNo = "'" + orderNoValue;
    ...
    public static async normalSales(client: any, payMethods: any, prices: any, up_down: number) {
        for (let j = 0; j < payMethods.length; j++) {
            // 恢复正常等待时间
            ...
            if (i <= 6) {
                if (up_down === 0) {
                    await TouchAction.clickPay(client, i);
                } else {
                ...
                if (up_down < downCount) {
                    ...
                    if (payss.length - 6 * downCount < 3) {
                        num += 4;
                        await TouchAction.clickPay(client, num);
            ...
            // 输入该支付方式的价格
            await TouchAction.touchPriceAction(client, prices[j].toString());
            let confirm = await client.$('//android.widget.Button[@content-desc="确定"]');
            await confirm.click();

            // 各种支付方式的处理
            await PaymentProcessing.handle(client);
        }
        // 恢复正常等待时间
    ...
    public static async sales_cancelled(client: any, payMethods: any, prices: any, j: number, up_down: number) {
        // 恢复正常等待时间
        await client.setImplicitTimeout(10000);
        let payName: string = payMethods[j];
        //获取所有支付方式
